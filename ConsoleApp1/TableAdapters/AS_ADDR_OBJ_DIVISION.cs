...
{
    internal class AS_ADDR_OBJ_DIVISION : IDisposable 
    {
   public const string SQL_TABLE_NAME = "AS_ADDR_OBJ_DIVISION";
        public const string SQL_TABLE_TEMP_NAME = "AS_ADDR_OBJ_DIVISION_Temp";
        ...
        public AS_ADDR_OBJ_DIVISION(SqlConnection sqlConnection, string filesDirectory, string xmlFileName, bool useMerge)
        {
            _useMerge = useMerge;
            ...
            // Порядок колонок должен строго совпадать с порядком в проекте таблицы Sql, 
            // т.к. SqlBulkCopy ориентируется на порядок колонок при вставке данных
            _dt.Columns.Add("ID", typeof(string)).AllowDBNull = true;
            _dt.Columns.Add("PARENTID", typeof(string)).AllowDBNull = true;
            _dt.Columns.Add("CHILDID", typeof(string)).AllowDBNull = true;
            _dt.Columns.Add("CHANGEID", typeof(int)).AllowDBNull = true;
            ...
            var typeMap = new List<Type> { typeof(string), typeof(string), typeof(string), typeof(int) };
            ...
            using (SqlCommand cmd = new SqlCommand(string.Empty, _sqlConnection, trn))
            {
                ...
                for (int i = 0; i < _dt.Rows.Count; i++)
                {
                    ...
                    for (int k = 1; k < 100; k++)
                    {
                        try
                        {
                            cmd.CommandText = "DELETE FROM [dbo].[AS_ADDR_OBJ_DIVISION" +
        ...
        private string CreateTempTableCommand()
        {
            return
                "CREATE TABLE " + SQL_TABLE_TEMP_NAME + @" (
	                [ID] [nvarchar](19) NULL,
	                [PARENTID] [nvarchar](19) NULL,
	                [CHILDID] [nvarchar](19) NULL,
	                [CHANGEID] [int] NULL)";
        }
        ...
        {
            return
     "MERGE INTO [AS_ADDR_OBJ_DIVISION] AS [Target] USING " + SQL_TABLE_TEMP_NAME + " AS [Source] ON [Target].[ID] = [Source].[ID] " +
     ...
     VALUES
        ...
        public static IEnumerable<FileInfo> GetHouseFiles(string dir)
        {
            return Directory.GetFiles(dir, "*.xml").
                             ...
                             Where(x => x.Name.StartsWith("AS_ADDR_OBJ_DIVISION", StringComparison.InvariantCultureIgnoreCase) &&
