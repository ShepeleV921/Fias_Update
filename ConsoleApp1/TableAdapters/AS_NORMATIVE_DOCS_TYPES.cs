...
{
    internal class AS_NORMATIVE_DOCS_TYPES : IDisposable
    {
        public const string SQL_TABLE_NAME = "AS_NORMATIVE_DOCS_TYPES";
        public const string SQL_TABLE_TEMP_NAME = "AS_NORMATIVE_DOCS_TYPES_Temp";
        ...
        public AS_NORMATIVE_DOCS_TYPES(SqlConnection sqlConnection, string filesDirectory, string xmlFileName, bool useMerge)
        {
            ...
            var typeMap = new List<Type> { typeof(int), typeof(string), typeof(DateTime), typeof(DateTime) };
            ...
            using (SqlCommand cmd = new SqlCommand(string.Empty, _sqlConnection, trn))
            {
                ...
                for (int i = 0; i < _dt.Rows.Count; i++)
                {
                    ...
                    for (int k = 1; k < 100; k++)
                    {
                        try
                        {
                            cmd.CommandText = "DELETE FROM [dbo].[AS_NORMATIVE_DOCS_TYPES" +
        ...
        {
            return
     "MERGE INTO [AS_NORMATIVE_DOCS_TYPES] AS [Target] USING " + SQL_TABLE_TEMP_NAME + " AS [Source] ON [Target].[ID] = [Source].[ID] " +
     ...
     VALUES
        ...
        public static IEnumerable<FileInfo> GetHouseFiles(string dir)
        {
            return Directory.GetFiles(dir, "*.xml").
                             ...
                             Where(x => x.Name.StartsWith("AS_NORMATIVE_DOCS_TYPES", StringComparison.InvariantCultureIgnoreCase) &&
