...
{
    internal class AS_ROOMS : IDisposable
    {
        public const string SQL_TABLE_NAME = "AS_ROOMS";
        public const string SQL_TABLE_TEMP_NAME = "AS_ROOMS_Temp";
        ...
        public AS_ROOMS(SqlConnection sqlConnection, string filesDirectory, string xmlFileName, bool useMerge)
        {
            ...
            _dt.Columns.Add("ROOMTYPE", typeof(string)).AllowDBNull = true;
            ...
            using (SqlCommand cmd = new SqlCommand(string.Empty, _sqlConnection, trn))
            {
                ...
                for (int i = 0; i < _dt.Rows.Count; i++)
                {
                    ...
                    for (int k = 1; k < 100; k++)
                    {
                        try
                        {
                            cmd.CommandText = "DELETE FROM [dbo].[AS_ROOMS" +
        ...
        private string CreateTempTableCommand()
        {
            return
                "CREATE TABLE " + SQL_TABLE_TEMP_NAME + @" (
	                ...
	                [ROOMTYPE] [nvarchar](9) NULL,
	                [OPERTYPEID] [nvarchar](9) NULL,
        ...
        {
            return
     "MERGE INTO [AS_ROOMS] AS [Target] USING " + SQL_TABLE_TEMP_NAME + " AS [Source] ON [Target].[ID] = [Source].[ID] " +
     ...
     VALUES
        ...
        public static IEnumerable<FileInfo> GetHouseFiles(string dir)
        {
            return Directory.GetFiles(dir, "*.xml").
                             ...
                             Where(x => x.Name.StartsWith("AS_ROOMS", StringComparison.InvariantCultureIgnoreCase) &&
