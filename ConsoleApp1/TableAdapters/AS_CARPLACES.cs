...
{
    internal class AS_CARPLACES : IDisposable
    {
        public const string SQL_TABLE_NAME = "AS_CARPLACES";
        public const string SQL_TABLE_TEMP_NAME = "AS_CARPLACES_Temp";
        ...
        public AS_CARPLACES(SqlConnection sqlConnection, string filesDirectory, string xmlFileName, bool useMerge)
        {
            ...
            var typeMap = new List<Type> { typeof(string), typeof(string), typeof(string), typeof(string), typeof(string), typeof(string), typeof(string), typeof(string), typeof(DateTime), typeof(DateTime), typeof(DateTime), typeof(Boolean), typeof(Boolean) };
            ...
            using (SqlCommand cmd = new SqlCommand(string.Empty, _sqlConnection, trn))
            {
                ...
                for (int i = 0; i < _dt.Rows.Count; i++)
                {
                    ...
                    for (int k = 1; k < 100; k++)
                    {
                        try
                        {
                            cmd.CommandText = "DELETE FROM [dbo].[AS_CARPLACES" +
        ...
        {
            return
     "MERGE INTO [AS_CARPLACES] AS [Target] USING " + SQL_TABLE_TEMP_NAME + " AS [Source] ON [Target].[ID] = [Source].[ID] " +
     ...
     VALUES
        ...
        public static IEnumerable<FileInfo> GetHouseFiles(string dir)
        {
            return Directory.GetFiles(dir, "*.xml").
                             ...
                             Where(x => x.Name.StartsWith("AS_CARPLACES", StringComparison.InvariantCultureIgnoreCase) &&
