...
{
    internal class AS_REESTR_OBJECTS : IDisposable
    {
        public const string SQL_TABLE_NAME = "AS_REESTR_OBJECTS";
        public const string SQL_TABLE_TEMP_NAME = "AS_REESTR_OBJECTS_Temp";
        ...
        public AS_REESTR_OBJECTS(SqlConnection sqlConnection, string filesDirectory, string xmlFileName, bool useMerge)
        {
            ...
            _dt.Columns.Add("LEVELID", typeof(string)).AllowDBNull = true;
            _dt.Columns.Add("CREATEDATE", typeof(DateTime)).AllowDBNull = true;
            ...
            var typeMap = new List<Type> { typeof(string), typeof(string), typeof(string), typeof(Boolean), typeof(string), typeof(DateTime), typeof(DateTime) };
        ...
        public void DeleteFromServer()
        {
            ...
            {
                SqlParameter p = cmd.Parameters.Add("@OBJECTID", SqlDbType.UniqueIdentifier);

                for (int i = 0; i < _dt.Rows.Count; i++)
                {
                    int qResult = 0;
                    p.Value = _dt.Rows[i]["OBJECTID"];

                    for (int k = 1; k < 100; k++)
                    {
                        try
                        {
                            cmd.CommandText = "DELETE FROM [dbo].[OBJECTID" +
                                k.ToString().PadLeft(2, '0') + "] WHERE [OBJECTID] = @OBJECTID";
        ...
        {
            return
     "MERGE INTO [AS_REESTR_OBJECTS] AS [Target] USING " + SQL_TABLE_TEMP_NAME + " AS [Source] ON [Target].[OBJECTID] = [Source].[OBJECTID] " +
     ...
     VALUES
        ...
        public static IEnumerable<FileInfo> GetHouseFiles(string dir)
        {
            return Directory.GetFiles(dir, "*.xml").
                             ...
                             Where(x => x.Name.StartsWith("AS_REESTR_OBJECTS", StringComparison.InvariantCultureIgnoreCase) &&
