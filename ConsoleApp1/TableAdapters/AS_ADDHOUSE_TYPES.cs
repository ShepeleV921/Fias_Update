...
{
    internal class AS_ADDHOUSE_TYPES : IDisposable
    {
        public const string SQL_TABLE_NAME = "AS_ADDHOUSE_TYPES";
        public const string SQL_TABLE_TEMP_NAME = "AS_ADDHOUSE_TYPES_Temp";
        ...
        public AS_ADDHOUSE_TYPES(SqlConnection sqlConnection, string filesDirectory, string xmlFileName, bool useMerge)
        {
            _useMerge = useMerge;
            ...
            _dt.Columns.Add("SHORTNAME", typeof(string)).AllowDBNull = true;
            _dt.Columns.Add("DESC", typeof(string)).AllowDBNull = true;
            ...
            var typeMap = new List<Type> { typeof(string), typeof(string), typeof(string), typeof(string), typeof(Boolean), typeof(DateTime), typeof(DateTime), typeof(DateTime) };
    ...
    public string GetTablePath()
            ...
            using (SqlCommand cmd = new SqlCommand(string.Empty, _sqlConnection, trn))
            {
                ...
                for (int i = 0; i < _dt.Rows.Count; i++)
                {
                    ...
                    for (int k = 1; k < 100; k++)
                    {
                        try
                        {
                            cmd.CommandText = "DELETE FROM [dbo].[AS_ADDHOUSE_TYPES" +
        ...
        {
            return
     "MERGE INTO [AS_ADDHOUSE_TYPES] AS [Target] USING " + SQL_TABLE_TEMP_NAME + " AS [Source] ON [Target].[ID] = [Source].[ID] " +
     ...
     VALUES
        ...
        public static IEnumerable<FileInfo> GetHouseFiles(string dir)
        {
            return Directory.GetFiles(dir, "*.xml").
                             ...
                             Where(x => x.Name.StartsWith("AS_ADDHOUSE_TYPES", StringComparison.InvariantCultureIgnoreCase) &&
