...
{
    internal class AS_APARTMENTS : IDisposable
    {
        public const string SQL_TABLE_NAME = "AS_APARTMENTS";
        public const string SQL_TABLE_TEMP_NAME = "AS_APARTMENTS_Temp";
        ...
        public AS_APARTMENTS(SqlConnection sqlConnection, string filesDirectory, string xmlFileName, bool useMerge)
        {
            ...
            _dt.Columns.Add("APARTTYPE", typeof(string)).AllowDBNull = true;
            ...
            var typeMap = new List<Type> { typeof(string), typeof(string), typeof(string), typeof(string), typeof(string), typeof(string), typeof(string), typeof(string), typeof(string), typeof(DateTime), typeof(DateTime), typeof(DateTime), typeof(Boolean), typeof(Boolean) };
            ...
            using (SqlCommand cmd = new SqlCommand(string.Empty, _sqlConnection, trn))
            {
                ...
                for (int i = 0; i < _dt.Rows.Count; i++)
                {
                    ...
                    for (int k = 1; k < 100; k++)
                    {
                        try
                        {
                            cmd.CommandText = "DELETE FROM [dbo].[AS_APARTMENTS" +
        ...
        private string CreateTempTableCommand()
        {
            return
                "CREATE TABLE " + SQL_TABLE_TEMP_NAME + @" (
	                ...
	                [APARTTYPE] [nvarchar](2) NULL,
	                [OPERTYPEID] [nvarchar](2) NULL,
        ...
        {
            return
     "MERGE INTO [AS_APARTMENTS] AS [Target] USING " + SQL_TABLE_TEMP_NAME + " AS [Source] ON [Target].[ID] = [Source].[ID] " +
    @"WHEN MATCHED THEN UPDATE SET 
					 ...
					 [Target].[APARTTYPE]        = [Source].[APARTTYPE],
     ...
     VALUES
        ...
        public static IEnumerable<FileInfo> GetHouseFiles(string dir)
        {
            return Directory.GetFiles(dir, "*.xml").
                             ...
                             Where(x => x.Name.StartsWith("AS_APARTMENTS", StringComparison.InvariantCultureIgnoreCase) &&
