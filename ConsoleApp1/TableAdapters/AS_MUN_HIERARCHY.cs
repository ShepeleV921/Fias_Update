...
{
    internal class AS_MUN_HIERARCHY : IDisposable
    {
        public const string SQL_TABLE_NAME = "AS_MUN_HIERARCHY";
        public const string SQL_TABLE_TEMP_NAME = "AS_MUN_HIERARCHY_Temp";
        ...
        public AS_MUN_HIERARCHY(SqlConnection sqlConnection, string filesDirectory, string xmlFileName, bool useMerge)
        {
            ...
            _dt.Columns.Add("OKTMO", typeof(string)).AllowDBNull = true;
            ...
            var typeMap = new List<Type> { typeof(string), typeof(string), typeof(string), typeof(string), typeof(string), typeof(string), typeof(string), typeof(DateTime), typeof(DateTime), typeof(DateTime), typeof(Boolean), typeof(string) };
            ...
            using (SqlCommand cmd = new SqlCommand(string.Empty, _sqlConnection, trn))
            {
                ...
                for (int i = 0; i < _dt.Rows.Count; i++)
                {
                    ...
                    for (int k = 1; k < 100; k++)
                    {
                        try
                        {
                            cmd.CommandText = "DELETE FROM [dbo].[AS_MUN_HIERARCHY" +
        ...
        {
            return
     "MERGE INTO [AS_MUN_HIERARCHY] AS [Target] USING " + SQL_TABLE_TEMP_NAME + " AS [Source] ON [Target].[ID] = [Source].[ID] " +
    @"WHEN MATCHED THEN UPDATE SET 
					 [Target].[ID]   = [Source].[ID],
					 [Target].[OBJECTID]       = [Source].[OBJECTID],
					 [Target].[PARENTOBJID]   = [Source].[PARENTOBJID],
					 [Target].[CHANGEID]       = [Source].[CHANGEID],
					 [Target].[OKTMO]   = [Source].[OKTMO],
     ...
     VALUES
        ...
        public static IEnumerable<FileInfo> GetHouseFiles(string dir)
        {
            return Directory.GetFiles(dir, "*.xml").
                             ...
                             Where(x => x.Name.StartsWith("AS_MUN_HIERARCHY", StringComparison.InvariantCultureIgnoreCase) &&
